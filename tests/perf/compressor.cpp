...
#pragma omp parallel for reduction(+ : error)
    ...
    const double max =
        (1.f - quality) * std::numeric_limits<T>::max() * double(nElem);
    TESTINFO(error <= max,
         ...
         i != names.end(); ++i)
    {
        ...
        while (true)
        {
            ...
            for (eq::StringsCIter j = images.begin(); j != images.end(); ++j)
            {
...
#ifndef NDEBUG
                // touch memory once
                destImage.setPixelData(buffer, image.compressPixelData(buffer));
                // force recompression
                ...
                destImage.writeImage(lunchbox::getDirname(filename) + "/out_" +
                ...
                case 1:
                    _compare<uint8_t>(data, destData, buffer,
                                      image.getAlphaUsage(), nElem, quality);
                    break;
                case 2:
                    if (quality == 1.f)
