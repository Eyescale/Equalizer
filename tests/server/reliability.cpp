...
public:
    ...
    virtual eq::Pipe* createPipe(eq::Node* parent) { return new Pipe(parent); }
...
int main(const int argc, char** argv)
{
    ...
    if (_checkGPU(client))
    {
        lunchbox::Strings configs =
            lunchbox::searchDirectory("reliability", ".*\\.eqc");
...
void _testConfig(eq::ClientPtr client, const std::string& filename)
{
    ...
    TESTINFO(nDraw == drawCalls, filename << ": " << nDraw
                                          << " != " << drawCalls);
    TESTINFO(nReadback == readbackCalls, filename << ": " << nReadback
                                                  << " != " << readbackCalls);
    TESTINFO(nAssemble == assembleCalls, filename << ": " << nAssemble
                                                  << " != " << assembleCalls);
    drawCalls = 0;
    readbackCalls = 0;
    assembleCalls = 0;
