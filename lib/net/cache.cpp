...
template< typename K > Cache< K >::Cache( const long maxSize )
        : _maxSize( maxSize )
        , _size( 0 )
{}

template< typename K > Cache< K >::~Cache()
{
...
template< typename K > Cache< K >::Item::Item()
        : command( 0 )
        , used( 0 )
        , pinned( 0 )
{}
...
template< typename K > 
bool Cache< K >::add( const K& key, Command* const command, const bool fixed )
{
#ifdef EQ_INSTRUMENT_CACHE
    ++nWrite;
#endif
    ...
    if( i != _data.end( ))
    {
        EQASSERTINFO( !fixed, "Failed to add a pinned item to the cache, " <<
                      "key already exists" );
        return false;
    }
...
template< typename K > 
Command* Cache< K >::operator[]( const K& key )
{
#ifdef EQ_INSTRUMENT_CACHE
    ++nRead;
#endif
...
template< typename K > 
bool Cache< K >::unpin( const K& key )
{
    base::ScopedMutex mutex( _mutex );
    typename Data::iterator i = _data.find( key );
    if( i == _data.end( ))
...
template< typename K > 
bool Cache< K >::erase( const K& key )
{
    base::ScopedMutex mutex( _mutex );
    typename Data::iterator i = _data.find( key );
    if( i == _data.end( ))
...
void Cache< K >::_releaseItems()
{
    ...
    if( _size > target )
        EQWARN << "Overfull cache, too many pinned items" << std::endl;
