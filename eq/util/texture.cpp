                                  ...
                                  const fabric::PixelViewport& pvp)
{
    ...
    EQ_GL_CALL(glCopyTexSubImage2D(_impl->target, 0, 0, 0, pvp.x, pvp.y, pvp.w,
                                   pvp.h));
                        ...
                        const int32_t height, const int32_t samples)
{
    ...
    EQ_GL_CALL(glFramebufferTexture2DEXT(GL_FRAMEBUFFER, target, _impl->target,
                                         _impl->name, 0));

    if (samples > 1)
    {
        EQ_GL_CALL(glTexImage2DMultisample(_impl->target, samples,
                                           _impl->internalFormat, width, height,
                                           false));
    }
...
void Texture::writeRGB(const std::string& filename) const
{
    ...
    image.setPixelViewport(
        eq::PixelViewport(0, 0, _impl->width, _impl->height));
...
const GLEWContext* Texture::glewGetContext() const
{
...
{
    return os << "Texture " << texture.getWidth() << "x" << texture.getHeight()
