...
protected:
    ...
    virtual void _removeChild(const uint128_t&) { LBUNIMPLEMENTED; }
    ...
    /** @internal */
    EQFABRIC_INL EventOCommand sendError(co::NodePtr node, const uint32_t event,
                                         const Error& error);
    ...
    /** @internal */
    virtual VisitorResult _acceptCompounds(V&) { return TRAVERSE_CONTINUE; }
    /** @internal */
...
private:
    ...
    enum DirtyBits
    {
        ...
        DIRTY_CONFIG_BITS = DIRTY_MEMBER | DIRTY_ATTRIBUTES | DIRTY_OBSERVERS |
                            DIRTY_LAYOUTS | DIRTY_CANVASES | DIRTY_NODES |
                            DIRTY_LATENCY
    };
...
template <class S, class C, class O, class L, class CV, class N, class V>
EQFABRIC_INL std::ostream& operator<<(
    std::ostream& os, const Config<S, C, O, L, CV, N, V>& config);
}
}
