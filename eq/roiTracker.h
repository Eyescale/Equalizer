...
public:
    ...
    /**
     * Has to be called once before any ROI calculation. Tells wether ROIFinder
     * should be executed for a particular region or not.  If true is returned,
     * then updateDelay must be called once before any next useROIFinder
     * call. If false is returned, then updateDelay shouldn't be called.
     *
     * This function uses _prvFrame areas data to match with current pvp. If
     * good intersection with one of previous areas is found, it will use
     * information from that area form previous frame to decide wether it should
     * skip ROIFinder execution or not. If good intersection is not found area
     * cosidered new, and function returns true.
     *
     ...
     *
     * It checks how much space ROIFinder was able to discard and if it is not
     * enough it will disable usage of ROIFinder for several next frames. If
     * ROIFinder was able to cut-off enought it will reset failure statistics
     * for that region.
     *
