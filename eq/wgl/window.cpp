...
public:
    ...
    WGLEWContext* wglewGetContext() { return _wglPipe.wglewGetContext(); }
    /**
...
HWND Window::_createWGLWindow(const PixelViewport& pvp)
{
    ...
    if (!RegisterClass(&wc))
    {
        sendError(ERROR_WGLWINDOW_REGISTERCLASS_FAILED) << lunchbox::sysError();
        return false;
    }
    ...
    HWND hWnd =
        CreateWindowEx(windowStyleEx, wc.lpszClassName,
                       name.empty() ? "Equalizer" : name.c_str(), windowStyle,
                       rect.left, rect.top, rect.right - rect.left,
                       rect.bottom - rect.top, 0, 0, // parent, menu
                       instance, 0);
    if (!hWnd)
    {
        sendError(ERROR_WGLWINDOW_CREATEWINDOW_FAILED) << lunchbox::sysError();
        return false;
    }
...
int Window::chooseWGLPixelFormat()
{
    ...
    const int pixelFormat = (WGLEW_ARB_pixel_format)
                                ? _chooseWGLPixelFormatARB(pfDC)
                                : _chooseWGLPixelFormat(pfDC);
...
int Window::_chooseWGLPixelFormatARB(HDC pfDC)
{
    ...
    while (true)
    {
        int pixelFormat = 0;
        UINT nFormats = 0;
        if (!wglChoosePixelFormatARB(pfDC, &attributes[0], 0, 1, &pixelFormat,
                                     &nFormats))
        {
        ...
        std::vector<GLint>::iterator iter =
...
HGLRC Window::createWGLContext()
{
    ...
    if (wglCreateContextAttribsARB && coreContext)
    {
        ...
        context = wglCreateContextAttribsARB(_impl->_wglAffinityDC
                                                 ? _impl->_wglAffinityDC
                                                 : _impl->_wglDC,
                                             shareCtx, attribList);
    }
    else
    {
        context = wglCreateContext(_impl->_wglAffinityDC ? _impl->_wglAffinityDC
                                                         : _impl->_wglDC);
    }

    if (!context)
    {
        sendError(ERROR_WGLWINDOW_CREATECONTEXT_FAILED) << lunchbox::sysError();
        return 0;
    }
