...
GLXFBConfig* Window::chooseGLXFBConfig()
{
    ...
    PFNGLXCHOOSEFBCONFIGSGIXPROC chooseFBConfig =
        GLXEW_VERSION_1_3 ? glXChooseFBConfig : glXChooseFBConfigSGIX;

    const int screen = DefaultScreen(_impl->xDisplay);
    int nConfigs = 0;
    GLXFBConfig* configs =
        chooseFBConfig(_impl->xDisplay, screen, &attributes[0], &nConfigs);
    ...
    while ((nConfigs == 0 || !configs) && !backoffAttributes.empty())
    {
        ...
        std::vector<int>::iterator iter =
            find(attributes.begin(), attributes.end(), attribute);
...
GLXContext Window::createGLXContext(GLXFBConfig* fbConfig)
{
    ...
    {
        int attribList[] = {GLX_CONTEXT_MAJOR_VERSION_ARB,
    ...
    else
    {
        if (GLXEW_VERSION_1_3)
            ...
            context =
                glXCreateContextWithConfigSGIX(_impl->xDisplay, fbConfig[0],
                                               type, shCtx, True);
    }
    ...
    if (!context)
    {
        ...
        if (!visInfo)
        {
            ...
            const int screen = DefaultScreen(_impl->xDisplay);
            visInfo =
                glXChooseVisual(_impl->xDisplay, screen, &attributes.front());
            if (!visInfo)
            {
...
XID Window::_createGLXWindow(GLXFBConfig* fbConfig, const PixelViewport& pvp)
{
    ...
    wa.colormap =
        XCreateColormap(_impl->xDisplay, parent, visInfo->visual, AllocNone);
    ...
    default:
        wa.override_redirect =
            getIAttribute(IATTR_HINT_FULLSCREEN) == ON ? True : False;
        break;
    }

    XID drawable =
        XCreateWindow(_impl->xDisplay, parent, pvp.x, pvp.y, pvp.w, pvp.h, 0,
...
void Window::setXDrawable(XID drawable)
{
    ...
    case UNDEFINED:
    {
        ...
        int x, y;
        ::Window childReturn;
        XTranslateCoordinates(_impl->xDisplay, parent, root, wa.x, wa.y, &x, &y,
                              &childReturn);
...
void Window::_resize(const PixelViewport& pvp)
{
    if (_impl->xDisplay && _impl->xDrawable)
        XMoveResizeWindow(_impl->xDisplay, _impl->xDrawable, pvp.x, pvp.y,
                          pvp.w, pvp.h);
}
                          ...
                          PointerEvent& event)
{
    ...
    case EVENT_WINDOW_POINTER_BUTTON_PRESS:
        ...
        {
            const unsigned eventMask =
                ButtonPressMask | ButtonReleaseMask | ButtonMotionMask;
            const int result =
                XGrabPointer(getXDisplay(), getXDrawable(), False, eventMask,
