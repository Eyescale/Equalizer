...
bool Config::update()
{
    ...
    lunchbox::Request<uint32_t> reqFinish = client->registerRequest<uint32_t>();
    lunchbox::Request<bool> request = client->registerRequest<bool>();
    send(getServer(), fabric::CMD_CONFIG_UPDATE) << reqVersion << reqFinish
                                                 << request;
...
void Config::_frameStart()
{
    ...
    {
        const int64_t age =
            _impl->frameTimes.back() - _impl->frameTimes.front();
...
uint32_t Config::finishFrame()
{
    ...
    const uint32_t frameToFinish =
        (_impl->currentFrame >= latency) ? _impl->currentFrame - latency : 0;
    ...
    case Statistic::PIPE_IDLE:
    {
        ...
        else
        {
            ...
            if (pos == std::string::npos) // append new pipe
                text << string << ", " << stat.resourceName << ' ' << idle
                     << "%";
...
void Config::setupMessagePump(Pipe* pipe)
{
    ...
    ClientPtr client = getClient();
    CommandQueue* queue =
        LBSAFECAST(CommandQueue*, client->getMainThreadQueue());
    LBASSERT(queue);
...
bool Config::_cmdFrameFinish(co::ICommand& cmd)
{
    ...
    LBLOG(LOG_TASKS) << "frame finish " << command << " frame "
                     << _impl->finishedFrame << std::endl;
...
bool Config::_cmdSwapObject(co::ICommand& cmd)
{
    ...
    const uint32_t requestID = command.read<uint32_t>();
    co::Object* object = reinterpret_cast<co::Object*>(command.read<void*>());
...
template EQFABRIC_API std::ostream& eq::fabric::operator<<(
    std::ostream&, const eq::Config::Super&);
/** @endcond */
...
#define CONST_FIND_ID_TEMPLATE2(type)                          \
    template EQ_API const type* eq::Config::Super::find<type>( \
        const uint128_t&) const;
...
#define FIND_NAME_TEMPLATE1(type)                                          \
    template EQ_API void eq::Config::Super::find<type>(const std::string&, \
                                                       const type**) const;
...
#define FIND_NAME_TEMPLATE2(type) \
    template EQ_API type* eq::Config::Super::find<type>(const std::string&);
