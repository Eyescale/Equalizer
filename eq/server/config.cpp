...
template <class T>
bool Config::_postDelete(const uint128_t& id)
{
    T* child = find<T>(id);
    if (!child)
        return false;
...
Channel* Config::getChannel(const ChannelPath& path)
{
    ...
    LBASSERTINFO(nodes.size() > path.nodeIndex, nodes.size() << " <= "
                                                             << path.nodeIndex);
...
void Config::_updateCanvases()
{
    ...
    for (Canvases::const_iterator i = canvases.begin(); i != canvases.end();
         ++i)
    {
        Canvas* canvas = *i;
...
void Config::_stopNodes()
{
    ...
    for (Nodes::const_iterator i = nodes.begin(); i != nodes.end(); ++i)
    {
        ...
        netNode->send(fabric::CMD_SERVER_DESTROY_CONFIG) << getID()
                                                         << LB_UNDEFINED_UINT32;
...
void Config::_syncClock()
{
    ...
    for (Nodes::const_iterator i = nodes.begin(); i != nodes.end(); ++i)
    {
        ...
        if (node->isRunning() || node->isApplicationNode())
        {
            ...
            send(netNode, fabric::CMD_CONFIG_SYNC_CLOCK)
                << getServer()->getTime();
        }
    }
}
...
bool Config::exit()
{
    ...
    EventOCommand cmd(send(findApplicationNetNode(), fabric::CMD_CONFIG_EVENT));
    Event event;
...
void Config::_startFrame(const uint128_t& frameID)
{
    ...
    if (appNode) // release appNode local sync
        send(appNode, fabric::CMD_CONFIG_RELEASE_FRAME_LOCAL) << _currentFrame;
...
void Config::notifyNodeFrameFinished(const uint32_t frameNumber)
{
    ...
    send(findApplicationNetNode(), fabric::CMD_CONFIG_FRAME_FINISH)
        << frameNumber;
    LBLOG(LOG_TASKS) << "TASK config frame finished  "
...
bool Config::_cmdInit(co::ICommand& cmd)
{
    ...
    sync();
    LBDEBUG << "Config init " << (result ? "successful" : "failed")
            << std::endl;

    const uint128_t version = commit();
    send(command.getRemoteNode(), fabric::CMD_CONFIG_INIT_REPLY)
        << version << requestID << result;
    return true;
}
...
bool Config::_cmdExit(co::ICommand& cmd)
{
    ...
    LBDEBUG << "Config exit " << (result ? "successful" : "failed")
            << std::endl;
...
bool Config::_cmdUpdate(co::ICommand& cmd)
{
    ...
    send(node, fabric::CMD_CONFIG_UPDATE_VERSION) << getVersion() << versionID
                                                  << finishID << request;
    ...
    for (Nodes::const_iterator i = nodes.begin(); i != nodes.end(); ++i)
    {
        ...
        if (node->isRunning() && node->isActive())
        {
            ...
            const int64_t interval =
                getServer()->getTime() - netNode->getLastReceiveTime();
...
#define FIND_NAME_TEMPLATE2(type) \
    template type* eq::server::Config::Super::find<type>(const std::string&);
...
#define CONST_FIND_NAME_TEMPLATE2(type)                         \
    template const type* eq::server::Config::Super::find<type>( \
