                                    ...
                                    const uint32_t frameNumber)
{
    ...
    if (!_tree)
    {
        ...
        {
        case 0:
            return; // no child compounds, can't do anything.
...
void LoadEqualizer::notifyLoadData(Channel* channel, const uint32_t frameNumber,
                                   const Statistics& statistics,
                                   const Viewport& region)
{
    LBLOG(LOG_LB2) << statistics.size() << " samples from "
                   << channel->getName() << " @ " << frameNumber << std::endl;
...
void LoadEqualizer::_update(Node* node, const Viewport& vp, const Range& range)
{
    if (!node)
        return;
                                ...
                                const Range& range)
{
    ...
    else
    {
        ...
        case MODE_VERTICAL:
            ...
            node->boundary2i.y() =
                LB_MAX(left->boundary2i.y(), right->boundary2i.y());
            node->boundaryf = LB_MAX(left->boundaryf, right->boundaryf);
            node->resistance2i.x() =
                LB_MAX(left->resistance2i.x(), right->resistance2i.x());
            node->resistance2i.y() =
                LB_MAX(left->resistance2i.y(), right->resistance2i.y());
        ...
        case MODE_HORIZONTAL:
            ...
            node->boundary2i.x() =
                LB_MAX(left->boundary2i.x(), right->boundary2i.x());
...
void LoadEqualizer::_computeSplit()
{
...
#ifndef NDEBUG
        for (LBDatas::const_iterator i = xData.begin(); i != xData.end(); ++i)
        {
            const Data& data = *i;
    ...
    const float time = float(_getTotalTime());
    LBLOG(LOG_LB2) << "Render time " << time << " for " << _tree->resources
                   << " resources" << std::endl;
...
void LoadEqualizer::_computeSplit(Node* node, const float time, LBDatas* datas,
                                  const Viewport& vp, const Range& range)
{
    ...
    LBASSERTINFO(node->resources > 0.f || !vp.hasArea() || !range.hasData(),
                 "Assigning " << node->resources << " work to viewport " << vp
                              << ", " << range);
    ...
    const float leftTime = node->resources > 0
                               ? time * node->left->resources / node->resources
                               : 0.f;
    ...
    case MODE_VERTICAL:
    {
        ...
        while (timeLeft > std::numeric_limits<float>::epsilon() &&
               splitPos < end)
        {
            LBLOG(LOG_LB2) << timeLeft << "ms left using " << workingSet.size()
                           << " tiles" << std::endl;
                 ...
                 i != workingSet.end(); ++i)
            {
                ...
                if (yContrib > 0.f)
                {
                    const float percentage =
                        (width / data.vp.w) * (yContrib / data.vp.h);
                    currentTime += (data.time * percentage);

                    LBLOG(LOG_LB2)
                        << data.vp << " contributes " << yContrib << " in "
                        << vp.h << " (" << percentage << ") with " << data.time
                        << ": " << (data.time * percentage) << " vp.y " << vp.y
                        << " dataEnd " << dataEnd << " vpEnd " << vpEnd
                        << std::endl;
                    LBASSERT(percentage < 1.01f)
                }
            }

            LBLOG(LOG_LB2) << splitPos << "..." << currentPos
                           << ": t=" << currentTime << " of " << timeLeft
                           << std::endl;
        ...
        if (getDamping() < 1.f)
            splitPos =
                (1.f - getDamping()) * splitPos + getDamping() * node->split;
        ...
        const float pvpW =
            static_cast<float>(root->getInheritPixelViewport().w);
        const float boundary = static_cast<float>(node->boundary2i.x()) / pvpW;
        ...
        LBLOG(LOG_LB2) << "Constrained split " << vp << " at X " << splitPos
                       << std::endl;
    ...
    case MODE_HORIZONTAL:
    {
               ...
               splitPos < end)
        {
                 ...
                 i != workingSet.end(); ++i)
            {
                ...
                {
                    const float percentage =
                        (height / data.vp.h) * (xContrib / data.vp.w);
                    currentTime += (data.time * percentage);

                    LBLOG(LOG_LB2)
                        << data.vp << " contributes " << xContrib << " in "
                        << vp.w << " (" << percentage << ") with " << data.time
                        << ": " << (data.time * percentage) << " total "
                        << currentTime << " vp.x " << vp.x << " dataEnd "
                        << dataEnd << " vpEnd " << vpEnd << std::endl;
                    LBASSERT(percentage < 1.01f)
                }
            }
        ...
        const float boundary = static_cast<float>(node->boundary2i.y()) / pvpH;
        ...
        LBLOG(LOG_LB2) << "Constrained split " << vp << " at Y " << splitPos
                       << std::endl;
    ...
    case MODE_DB:
    {
        ...
        const uint32_t ratio = static_cast<uint32_t>(splitPos / boundary + .5f);
        splitPos = ratio * boundary;
...
std::ostream& operator<<(std::ostream& os, const LoadEqualizer* lb)
{
    ...
    os << lunchbox::disableFlush << "load_equalizer" << std::endl
       << '{' << std::endl
