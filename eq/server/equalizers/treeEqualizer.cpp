                                    ...
                                    const uint32_t /*frame*/)
{
    ...
    if (!_tree)
    {
        ...
        {
        case 0:
            return; // no leaf compound, can't do anything.
...
void TreeEqualizer::notifyLoadData(Channel* channel, const uint32_t /*frame*/,
...
void TreeEqualizer::_update(Node* node)
{
    ...
    if (node->left->mode == MODE_2D)
    {
        ...
        node->left->mode =
            (node->mode == MODE_VERTICAL) ? MODE_HORIZONTAL : MODE_VERTICAL;
    ...
    else
    {
        ...
        case MODE_VERTICAL:
            node->maxSize.x() =
                node->left->maxSize.x() + node->right->maxSize.x();
            node->maxSize.y() =
                LB_MIN(node->left->maxSize.y(), node->right->maxSize.y());
            node->boundary2i.x() =
                node->left->boundary2i.x() + node->right->boundary2i.x();
            node->boundary2i.y() =
                LB_MAX(node->left->boundary2i.y(), node->right->boundary2i.y());
            node->boundaryf =
                LB_MAX(node->left->boundaryf, node->right->boundaryf);
            ...
            node->resistancef =
                LB_MAX(node->left->resistancef, node->right->resistancef);
            break;
        case MODE_HORIZONTAL:
            node->maxSize.x() =
                LB_MIN(node->left->maxSize.x(), node->right->maxSize.x());
            node->maxSize.y() =
                node->left->maxSize.y() + node->right->maxSize.y();
            node->boundary2i.x() =
                LB_MAX(node->left->boundary2i.x(), node->right->boundary2i.x());
            node->boundary2i.y() =
                node->left->boundary2i.y() + node->right->boundary2i.y();
            node->boundaryf =
                LB_MAX(node->left->boundaryf, node->right->boundaryf);
            node->resistance2i.x() = LB_MAX(node->left->resistance2i.x(),
                                            node->right->resistance2i.x());
...
void TreeEqualizer::_split(Node* node)
{
    ...
    LBLOG(LOG_LB2) << "Should split at " << split << " (" << target << ": "
                   << leftTime << " by " << left->resources << "/" << rightTime
                   << " by " << right->resources << ")" << std::endl;
...
void TreeEqualizer::_assign(Node* node, const Viewport& vp, const Range& range)
{
    LBLOG(LOG_LB2) << "assign " << vp << ", " << range << " time " << node->time
                   << " split " << node->split << std::endl;
    ...
    case MODE_VERTICAL:
    {
        ...
        LBLOG(LOG_LB2) << "Constrained split " << vp << " at X " << node->split
                       << std::endl;
    ...
    case MODE_DB:
    {
        ...
        float absoluteSplit =
            range.start + (range.end - range.start) * node->split;
        ...
        const float oldSplit =
            range.start + (range.end - range.start) * node->oldsplit;
        ...
        {
            node->split =
                (absoluteSplit - range.start) / (range.end - range.start);
            node->oldsplit = node->split;
        }
...
std::ostream& operator<<(std::ostream& os, const TreeEqualizer* lb)
{
    ...
    os << lunchbox::disableFlush << "tree_equalizer" << std::endl
       << '{' << std::endl
