                                       ...
                                       const RenderContext& context)
{
    ...
    if (compound->testInheritTask(fabric::TASK_DRAW))
    {
        ...
        _updated = true;
        LBLOG(LOG_TASKS) << "TASK draw " << _channel->getName() << " " << finish
                         << std::endl;
    }
}
...
void ChannelUpdateVisitor::_updateDrawFinish(const Compound* compound) const
{
    ...
    LBLOG(LOG_TASKS) << "TASK channel draw finish " << _channel->getName()
                     << " frame " << _frameNumber << " id " << _frameID
                     << std::endl;
    ...
    node->send(fabric::CMD_PIPE_FRAME_DRAW_FINISH, pipe->getID())
        << _frameID << _frameNumber;
    LBLOG(LOG_TASKS) << "TASK pipe draw finish " << pipe->getName() << " frame "
                     << _frameNumber << " id " << _frameID << std::endl;
...
fabric::ColorMask ChannelUpdateVisitor::_getDrawBufferMask(
    const Compound* compound) const
{
    ...
    {
    case EYE_LEFT:
        return ColorMask(compound->getInheritIAttribute(
                                            ...
                                            const RenderContext& context)
{
    ...
    // view start task
    LBLOG(LOG_TASKS) << "TASK view start " << _channel->getName() << std::endl;
